<?php
  ...
  $sqll=mysqli_query($db,"delete from image where ID='$id'");
  if ($sqll) {
    echo "<script>alert('Details updated successfully');</script>";
  }
    
  }
  $sql="INSERT INTO image(Photo,Description,Nutrients,Conditionn) VALUES('$Photo','$Description','$Nutrients','$Conditionn')";
  mysqli_query($db,$sql);
...
if (isset($_GET['id'])) {
  $id=$_GET['id'];
$res=mysqli_query($db,"delete from image where ID='$id'");
if($res){
 echo "<script type='text/javascript'>alert('Data deleted succussfully');
  window.location='leaf.php';</script>";
}
else{
...
<button type="submit" name="button" id="back"><a href="home1.php">Back</a> </button>
    ...
    <div id="content">
      <table width="100%" border="1" >
                          <th colspan="5" style="line-height:2px;"><h2>Leafy Vegetables</h2></th>
    ...
    <?php
      ...
      echo "<tr>";   
      echo "<td id='hh'><img src='images/".$row['Photo']."' height='100' width='150'> </td></th>";
      echo "<td width='20%'> <p>".$row['Description']."</p> </td>";
      echo "<td width='20%'> <p>".$row['Nutrients']."</p> </td>";
      echo "<td width='20%'> <p>".$row['Conditionn']."</p> </td>";
      echo "<td><button id='bt' onclick='return edit($var)'><a href='edit1.php?ID=$var'>EDIT</a></button>
...
<div id="form1">
    <form  method="POST" action="" enctype="multipart/form-data">
      <table align="center" border="1px" style="width: 100%; line-height: 45px;">
        <tr>
          <th colspan="8" style="line-height:2px;"><h2>Leafy Vegetables</h2></th>
        </tr>
        <t>
